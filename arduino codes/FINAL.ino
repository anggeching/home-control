...
void read_sensors() {
  ...
  for (int i = 0; i < DETECTION_THRESHOLD; i++) {
    ...
    if (sensorValue == LOW) { // Assuming the sensor outputs a value below 512 when an object is detected
      detectionCount++;
    }
  ...
  if (currentDetection) {
    // Object detected, start the timer if not already started
    if (!previousDetection) {
      detectionStartTime = currentMillis;
    }
    // Keep detection status true for 5 seconds
    ...
    // No object detected, reset detection
    detectionStartTime = currentMillis;
  }
  ...
  jsonDoc["irDetected"] = motionDetected; // Use motionDetected
  jsonDoc["light"] = light;
...
void webcontrol() {
  ...
  if (signalState == HIGH) {
    ...
    servo1.write(0); // If the control pin is low, rotate servo to 0 degrees
  }
