...
#define IR_SENSOR_PIN 4      // IR sensor pin (digital pin)
#define LDR_PIN A0     // Analog pin for LDR
#define LED_PIN A1     // Output pin for LED


// Constants
const unsigned long motionInterval = 5000; // Time interval to reset motion detection (5 seconds)
...
// Read IR sensor for motion detection
  ...
  bool irDetected = false; // Default to no motion

  if (sensorValue == HIGH && previousState == LOW) {
    // Transition from no obstacle to obstacle (simulates motion detected)
    Serial.println("Motion detected");
    irDetected = true; // Motion detected
    motionTime = millis(); // Record the time of motion detection
  } else if (sensorValue == LOW && previousState == HIGH) {
    // Transition from obstacle to no obstacle (simulates motion stopped)
    Serial.println("Motion stopped");
  }

  // Check if the motion detection should be reset after a certain interval
  if (millis() - motionTime > motionInterval) {
    Serial.println("No motion");
    irDetected = false; // No motion after interval
  }

  previousState = sensorValue; // Update the previous state
